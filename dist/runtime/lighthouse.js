 ...
 * ```
 */
export function isLighthouseInstruction(ix) {
 ...
 * ```
 */
export function isLighthouseError(error) {
    const msg = extractErrorMessage(error);
    if (!msg)
        return false;
    // Direct program ID reference
    if (msg.includes(LIGHTHOUSE_PROGRAM_ID_STR))
        return true;
    // 0x1900 hex error code (case-insensitive)
    if (/custom\s+program\s+error:\s*0x1900\b/i.test(msg))
        return true;
    // JSON InstructionError format: {"Custom": 6400}
    if (/"Custom"\s*:\s*6400\b/.test(msg) && /InstructionError/i.test(msg))
        return true;
    return false;
}
/**
 ...
 */
export function isLighthouseFailureInLogs(logs) {
 ...
 * ```
 */
export function stripLighthouseInstructions(instructions, percolatorProgramId) {
    ...
    if (percolatorProgramId) {
        const hasPercolatorIx = instructions.some((ix) => ix.programId.equals(percolatorProgramId));
        if (!hasPercolatorIx) {
 ...
 * ```
 */
export function stripLighthouseFromTransaction(transaction, percolatorProgramId) {
    // When a programId is provided, refuse to strip guards from transactions
    // that don't contain any Percolator instructions.
    if (percolatorProgramId) {
        const hasPercolatorIx = transaction.instructions.some((ix) => ix.programId.equals(percolatorProgramId));
        if (!hasPercolatorIx)
            return transaction;
    }
 ...
 */
export function countLighthouseInstructions(ixsOrTx) {
 ...
 */
export const LIGHTHOUSE_USER_MESSAGE = "Your wallet's transaction guard (Blowfish/Lighthouse) is blocking this transaction. " +
 ...
 */
export function classifyLighthouseError(error) {
    if (isLighthouseError(error)) {
