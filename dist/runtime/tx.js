import { TransactionInstruction, Transaction, ComputeBudgetProgram, } from "@solana/web3.js";
...
export async function simulateOrSend(params) {
    ...
    if (computeUnitLimit !== undefined) {
        if (typeof computeUnitLimit !== "number" ||
            !Number.isInteger(computeUnitLimit) ||
            computeUnitLimit < 1 ||
            computeUnitLimit > MAX_COMPUTE_UNIT_LIMIT) {
            throw new Error(`computeUnitLimit must be an integer in [1, ${MAX_COMPUTE_UNIT_LIMIT}]`);
        }
    }
 ...
 */
export function formatResult(result, jsonMode) {
    if (jsonMode) {
