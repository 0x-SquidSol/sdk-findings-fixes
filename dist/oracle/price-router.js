...
function parseDexScreenerPairs(json) {
    ...
    for (const pair of rawPairs) {
        ...
        const rawPrice = typeof priceUsd === "string" || typeof priceUsd === "number"
...
function parseJupiterMintEntry(json, mint) {
    ...
    if (!Number.isFinite(price) || price <= 0)
        return null;
    let mintSymbol = "?";
    if (typeof row.mintSymbol === "string")
...
// ---------------------------------------------------------------------------
export const PYTH_SOLANA_FEEDS = {
    // SOL
...
async function fetchDexSources(mint, signal) {
    try {
        const resp = await fetch(`https://api.dexscreener.com/latest/dex/tokens/${encodeURIComponent(mint)}`, {
        ...
        console.warn(`[fetchDexSources] Error fetching DexScreener data for mint ${mint}:`, err instanceof Error ? err.message : String(err));
        return [];
    }
}
// ---------------------------------------------------------------------------
...
async function fetchJupiterSource(mint, signal) {
    try {
        const resp = await fetch(`https://api.jup.ag/price/v2?ids=${encodeURIComponent(mint)}`, {
            signal: effectiveSignal(signal),
            headers: { "User-Agent": "percolator/1.0" },
        });
        if (!resp.ok) {
        ...
        console.warn(`[fetchJupiterSource] Error fetching Jupiter data for mint ${mint}:`, err instanceof Error ? err.message : String(err));
        return null;
    }
}
// ---------------------------------------------------------------------------
// Main resolver
// ---------------------------------------------------------------------------
export async function resolvePrice(mint, signal, options) {
