...
function requireDecimalUIntString(value, field) {
    ...
    if (!/^(0|[1-9]\d*)$/.test(t)) {
        throw new ValidationError(field, `"${value}" is not a valid non-negative integer (use decimal digits only, e.g. 123).`);
    }
    return t;
}
 ...
 * ```
 */
export function validatePublicKey(value, field) {
    try {
        return new PublicKey(value);
    }
    catch {
        throw new ValidationError(field, `"${value}" is not a valid base58 public key. ` +
 ...
 */
export function validateIndex(value, field) {
    const t = requireDecimalUIntString(value, field);
    const bi = BigInt(t);
    if (bi > BigInt(U16_MAX)) {
        throw new ValidationError(field, `must be <= ${U16_MAX} (u16 max), got ${t}`);
    }
 ...
 */
export function validateAmount(value, field) {
    const t = value.trim();
    if (!/^(0|[1-9]\d*)$/.test(t)) {
        throw new ValidationError(field, `"${value}" is not a valid non-negative integer. Use decimal digits only.`);
    }
    const num = BigInt(t);
    if (num < 0n) {
 ...
 */
export function validateU128(value, field) {
    const t = value.trim();
    if (!/^(0|[1-9]\d*)$/.test(t)) {
        throw new ValidationError(field, `"${value}" is not a valid non-negative integer. Use decimal digits only.`);
    }
 ...
 */
export function validateI64(value, field) {
    const t = value.trim();
    if (!/^-?(0|[1-9]\d*)$/.test(t)) {
        throw new ValidationError(field, `"${value}" is not a valid integer. Use decimal digits only, with optional leading minus.`);
    }
    const num = BigInt(t);
    if (num < I64_MIN) {
 ...
 */
export function validateI128(value, field) {
    const t = value.trim();
    if (!/^-?(0|[1-9]\d*)$/.test(t)) {
        throw new ValidationError(field, `"${value}" is not a valid integer. Use decimal digits only, with optional leading minus.`);
    }
 ...
 */
export function validateBps(value, field) {
    const t = requireDecimalUIntString(value, field);
    const bi = BigInt(t);
    if (bi > 10000n) {
        throw new ValidationError(field, `must be <= 10000 (100%), got ${t}`);
    }
 ...
 */
export function validateU64(value, field) {
    return validateAmount(value, field);
}
/**
 * Validate a u16 value.
 */
export function validateU16(value, field) {
    const t = requireDecimalUIntString(value, field);
    const bi = BigInt(t);
    if (bi > BigInt(U16_MAX)) {
        throw new ValidationError(field, `must be <= ${U16_MAX} (u16 max), got ${t}`);
