...
export function getProgramId(network) {
    ...
    if (!network) {
        ...
        if (override) {
            console.warn(`[percolator-sdk] PROGRAM_ID env override active: ${override} — ensure this points to a trusted program`);
 ...
 */
export function getMatcherProgramId(network) {
    if (!network) {
        const override = safeEnv("MATCHER_PROGRAM_ID");
        if (override) {
            console.warn(`[percolator-sdk] MATCHER_PROGRAM_ID env override active: ${override} — ensure this points to a trusted program`);
            return new PublicKey(override);
        }
    }
    // Use provided network or detect from env — default to devnet (never mainnet silently)
