 ...
 */
export function parseChainlinkPrice(data, options) {
    if (data.length < CHAINLINK_MIN_SIZE) {
        throw new Error(`Oracle account data too small: ${data.length} bytes (need at least ${CHAINLINK_MIN_SIZE})`);
    }
    ...
    if (options?.maxStalenessSeconds !== undefined && updatedAt > 0) {
        ...
        if (age > options.maxStalenessSeconds) {
            throw new Error(`Oracle price is stale: last updated ${age}s ago (max ${options.maxStalenessSeconds}s)`);
        }
    }
 ...
 */
export function isValidChainlinkOracle(data) {
    try {
