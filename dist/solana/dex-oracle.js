 ...
 */
export function detectDexType(ownerProgramId) {
    if (ownerProgramId.equals(PUMPSWAP_PROGRAM_ID))
        return "pumpswap";
    if (ownerProgramId.equals(RAYDIUM_CLMM_PROGRAM_ID))
        return "raydium-clmm";
    if (ownerProgramId.equals(METEORA_DLMM_PROGRAM_ID))
        return "meteora-dlmm";
    return null;
}
/**
 ...
 */
export function parseDexPool(dexType, poolAddress, data) {
    switch (dexType) {
        case "pumpswap":
 ...
 */
export function computeDexSpotPriceE6(dexType, data, vaultData) {
    switch (dexType) {
        case "pumpswap":
            if (!vaultData)
...
function computeRaydiumClmmPriceE6(data) {
    ...
    if (decimals0 > MAX_TOKEN_DECIMALS || decimals1 > MAX_TOKEN_DECIMALS) {
        throw new Error(`Raydium CLMM: decimals out of range (${decimals0}, ${decimals1}); max ${MAX_TOKEN_DECIMALS}`);
    }
...
function computeMeteoraDlmmPriceE6(data) {
    ...
    if (Math.abs(activeId) > MAX_ACTIVE_ID_ABS) {
        throw new Error(`Meteora DLMM: |activeId| ${Math.abs(activeId)} exceeds max ${MAX_ACTIVE_ID_ABS}`);
    }
    const MAX_ABS_BIN_ID = 500_000;
    if (activeId > MAX_ABS_BIN_ID || activeId < -MAX_ABS_BIN_ID) {
        throw new Error(`Meteora DLMM: activeId ${activeId} exceeds safe range (±${MAX_ABS_BIN_ID})`);
    }
    ...
    while (exp > 0n) {
        if (iterations++ >= MAX_ITERATIONS) {
            throw new Error(`Meteora DLMM: exponentiation loop exceeded ${MAX_ITERATIONS} iterations (activeId=${activeId})`);
        }
        if (exp & 1n) {
    ...
    if (isNeg) {
        ...
        return result / 1000000000000n; // 1e18 → 1e6
    }
}
// ============================================================================
// Helpers
