 ...
 */
export function deriveVaultAuthority(programId, slab) {
 ...
 */
export function deriveInsuranceLpMint(programId, slab) {
    return PublicKey.findProgramAddressSync([textEncoder.encode("ins_lp"), slab.toBytes()], programId);
}
 ...
 */
export function deriveLpPda(programId, slab, lpIdx) {
    if (typeof lpIdx !== "number" ||
        !Number.isInteger(lpIdx) ||
        lpIdx < 0 ||
        lpIdx > LP_INDEX_U16_MAX) {
        throw new Error(`deriveLpPda: lpIdx must be an integer in [0, ${LP_INDEX_U16_MAX}], got ${lpIdx}`);
    }
    const idxBuf = new Uint8Array(2);
    new DataView(idxBuf.buffer).setUint16(0, lpIdx, true);
    return PublicKey.findProgramAddressSync([textEncoder.encode("lp"), slab.toBytes(), idxBuf], programId);
}
/**
...
export function deriveKeeperFund(programId, slab) {
    return PublicKey.findProgramAddressSync([textEncoder.encode("keeper_fund"), slab.toBytes()], programId);
}
// ---------------------------------------------------------------------------
// DEX Program IDs
// ---------------------------------------------------------------------------
 ...
 * ```
 */
export function deriveCreatorLockPda(programId, slab) {
    return PublicKey.findProgramAddressSync([textEncoder.encode(CREATOR_LOCK_SEED), slab.toBytes()], programId);
}
...
export function derivePythPushOraclePDA(feedIdHex) {
    const normalized = normalizePythFeedIdHex(feedIdHex);
    if (!FEED_HEX_RE.test(normalized)) {
        throw new Error(`derivePythPushOraclePDA: feedIdHex must be 64 hex digits (32 bytes); got ${normalized.length === 64 ? "non-hexadecimal characters" : normalized.length + " chars"}`);
    }
    ...
    for (let i = 0; i < 32; i++) {
        ...
        if (Number.isNaN(byte)) {
            throw new Error(`derivePythPushOraclePDA: failed to parse hex byte at position ${i}: "${hexPair}"`);
        }
        feedId[i] = byte;
    }
