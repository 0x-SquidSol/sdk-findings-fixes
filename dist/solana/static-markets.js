 ...
 * ```
 */
export function getStaticMarkets(network) {
 ...
 * ```
 */
export function registerStaticMarkets(network, entries) {
    const existing = USER_MARKETS[network];
    const seen = new Set(existing.map(e => e.slabAddress));
    for (const entry of entries) {
        if (!entry.slabAddress)
            continue;
        if (seen.has(entry.slabAddress))
            continue;
        ...
        try {
            ...
            console.warn(`[registerStaticMarkets] Skipping invalid slabAddress: ${entry.slabAddress}`);
            continue;
        }
 ...
 */
export function clearStaticMarkets(network) {
    if (network) {
