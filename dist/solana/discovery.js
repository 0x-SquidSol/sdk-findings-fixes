...
import { parseHeader, parseConfig, parseParams, detectSlabLayout, SLAB_TIERS_V1M, SLAB_TIERS_V2, SLAB_TIERS_V_ADL, } from "./slab.js";
import { getStaticMarkets } from "./static-markets.js";
 ...
 */
export function slabDataSize(maxAccounts) {
 ...
 */
export function slabDataSizeV1(maxAccounts) {
 ...
 */
export function validateSlabTierMatch(dataSize, programSlabLen) {
 ...
 */
export async function discoverMarkets(connection, programId, options = {}) {
    const { sequential = false, interTierDelayMs = 200, rateLimitBackoffMs = [1_000, 3_000, 9_000, 27_000], maxParallelTiers = 6, } = options;
        ...
        for (let attempt = 0; attempt <= rateLimitBackoffMs.length; attempt++) {
            try {
                ...
                if (isRateLimitError(err) && attempt < rateLimitBackoffMs.length) {
                    ...
                    console.warn(`[discoverMarkets] 429 on tier dataSize=${tier.dataSize} attempt=${attempt + 1}, backing off ${delay}ms`);
                    await new Promise(r => setTimeout(r, delay));
                    continue;
                }
                // Non-429 or exhausted retries
                console.warn(`[discoverMarkets] Tier query failed (dataSize=${tier.dataSize}, attempt=${attempt + 1}):`, err instanceof Error ? err.message : err);
                return [];
            }
        }
        return [];
    ...
    try {
        if (sequential) {
            ...
            for (let offset = 0; offset < tiersToQuery.length; offset += effectiveMaxParallelTiers) {
                ...
                const queries = chunk.map(tier => connection.getProgramAccounts(programId, {
                    filters: [{ dataSize: tier.dataSize }],
                    dataSlice: { offset: 0, length: HEADER_SLICE_LENGTH },
                ...
                for (const result of results) {
                    if (result.status === "fulfilled") {
                        ...
                        console.warn("[discoverMarkets] Tier query rejected:", result.reason instanceof Error ? result.reason.message : result.reason);
                    }
                }
            }
        }
        ...
        console.warn("[discoverMarkets] dataSize filters failed, falling back to memcmp:", err instanceof Error ? err.message : err);
        try {
            const fallback = await connection.getProgramAccounts(programId, {
                filters: [
                    {
            ...
            console.warn("[discoverMarkets] memcmp fallback also failed:", memcmpErr instanceof Error ? memcmpErr.message : memcmpErr);
        }
    }
    ...
    if (rawAccounts.length === 0 && options.apiBaseUrl) {
        console.warn("[discoverMarkets] RPC discovery returned 0 markets, falling back to REST API");
        try {
            const apiResult = await discoverMarketsViaApi(connection, programId, options.apiBaseUrl, { timeoutMs: options.apiTimeoutMs });
            if (apiResult.length > 0) {
                return apiResult;
            }
            // API returned 0 markets — fall through to tier 3
            console.warn("[discoverMarkets] REST API returned 0 markets, checking tier-3 static bundle");
        }
        catch (apiErr) {
            console.warn("[discoverMarkets] API fallback also failed:", apiErr instanceof Error ? apiErr.message : apiErr);
    ...
    if (rawAccounts.length === 0 && options.network) {
        ...
        if (staticEntries.length > 0) {
            console.warn(`[discoverMarkets] Tier 1+2 failed, falling back to static bundle (${staticEntries.length} addresses for ${options.network})`);
            try {
                return await discoverMarketsViaStaticBundle(connection, programId, staticEntries);
            }
            catch (staticErr) {
                console.warn("[discoverMarkets] Static bundle fallback also failed:", staticErr instanceof Error ? staticErr.message : staticErr);
                // Fall through to return empty array
            }
        }
        else {
            console.warn(`[discoverMarkets] Static bundle has 0 entries for ${options.network} — skipping tier 3`);
        }
    }
    ...
    for (const { pubkey, account, maxAccounts, dataSize } of accounts) {
        ...
        if (!layout) {
            console.warn(`[discoverMarkets] Skipping account ${pkStr}: unrecognized layout for dataSize=${dataSize}`);
            continue;
        }
        try {
            ...
            console.warn(`[discoverMarkets] Failed to parse account ${pubkey.toBase58()}:`, err instanceof Error ? err.message : err);
        }
    }
    return markets;
}
 ...
 * ```
 */
export async function getMarketsByAddress(connection, programId, addresses, options = {}) {
    if (addresses.length === 0)
        return [];
    const { batchSize = 100, interBatchDelayMs = 0, } = options;
        ...
        for (let i = 0; i < batch.length; i++) {
            ...
            if (info && info.data) {
                if (!info.owner.equals(programId)) {
                    console.warn(`[getMarketsByAddress] Skipping ${batch[i].toBase58()}: owner mismatch ` +
    ...
    for (const entry of fetched) {
        ...
        if (!valid) {
            console.warn(`[getMarketsByAddress] Skipping ${pubkey.toBase58()}: invalid magic bytes`);
            continue;
        }
        // Detect layout from full account data length
        const layout = detectSlabLayout(data.length, data);
        if (!layout) {
            console.warn(`[getMarketsByAddress] Skipping ${pubkey.toBase58()}: unrecognized layout for dataSize=${data.length}`);
            continue;
        }
        try {
            const header = parseHeader(data);
            ...
            console.warn(`[getMarketsByAddress] Failed to parse account ${pubkey.toBase58()}:`, err instanceof Error ? err.message : err);
        }
    }
    return markets;
}
 ...
 * ```
 */
export async function discoverMarketsViaApi(connection, programId, apiBaseUrl, options = {}) {
    ...
    if (!response.ok) {
        throw new Error(`[discoverMarketsViaApi] API returned ${response.status} ${response.statusText} from ${url}`);
    }
    ...
    for (const entry of apiMarkets) {
        if (!entry.slab_address || typeof entry.slab_address !== "string")
            continue;
        try {
            addresses.push(new PublicKey(entry.slab_address));
        }
        catch {
            console.warn(`[discoverMarketsViaApi] Skipping invalid slab address: ${entry.slab_address}`);
        }
    }
    if (addresses.length === 0) {
    ...
    console.log(`[discoverMarketsViaApi] API returned ${addresses.length} slab addresses, fetching on-chain data`);
 ...
 * ```
 */
export async function discoverMarketsViaStaticBundle(connection, programId, entries, options = {}) {
    if (entries.length === 0)
        return [];
    // Extract valid slab addresses from static entries
    const addresses = [];
    for (const entry of entries) {
        if (!entry.slabAddress || typeof entry.slabAddress !== "string")
            continue;
        try {
            addresses.push(new PublicKey(entry.slabAddress));
        }
        catch {
            console.warn(`[discoverMarketsViaStaticBundle] Skipping invalid slab address: ${entry.slabAddress}`);
        }
    }
    if (addresses.length === 0) {
    ...
    console.log(`[discoverMarketsViaStaticBundle] Fetching ${addresses.length} slab addresses on-chain`);
