 ...
 */
export function detectSlabLayout(dataLen, data) {
    ...
    if (v1ln !== undefined)
        return buildLayout(1, v1ln, V1_ENGINE_OFF_LEGACY);
    return null;
}
/**
 ...
 */
export function detectLayout(dataLen, data) {
...
// Fetch
// =============================================================================
export async function fetchSlab(connection, slabPubkey) {
...
export const DEFAULT_OI_RAMP_SLOTS = 432000n;
export function computeEffectiveOiCapBps(config, currentSlot) {
    const target = config.oiCapMultiplierBps;
    if (target === 0n)
        return 0n;
    if (config.oiRampSlots === 0n)
        return target;
    if (target <= RAMP_START_BPS)
        return target;
    const elapsed = currentSlot > config.marketCreatedSlot
        ? currentSlot - config.marketCreatedSlot
        : 0n;
    if (elapsed >= config.oiRampSlots)
        return target;
...
// Header helpers
// =============================================================================
export function readNonce(data) {
 ...
 */
export function parseConfig(data, layoutHint) {
 ...
 * @param layoutHint - Pre-detected layout to use; if omitted, detected from data.length.
 */
export function parseParams(data, layoutHint) {
    const layout = layoutHint !== undefined ? layoutHint : detectSlabLayout(data.length, data);
 ...
 */
export function parseEngine(data) {
    const layout = detectSlabLayout(data.length, data);
    if (!layout) {
 ...
 */
export function parseUsedIndices(data) {
    const layout = detectSlabLayout(data.length, data);
    if (!layout)
        throw new Error(`Unrecognized slab data length: ${data.length}`);
 ...
 */
export function isAccountUsed(data, idx) {
    const layout = detectSlabLayout(data.length, data);
    if (!layout)
        return false;
    ...
    if (data.length < base + word * 8 + 8) {
        console.warn(`[isAccountUsed] Bitmap read would exceed buffer bounds: ` +
 ...
 */
export function maxAccountIndex(dataLen) {
 ...
 */
export function parseAccount(data, idx) {
    const layout = detectSlabLayout(data.length, data);
    if (!layout)
        throw new Error(`Unrecognized slab data length: ${data.length}`);
    ...
    if (base + layout.acctOwnerOff + 32 > data.length) {
        throw new Error(`Account owner field out of bounds: base=${base}, acctOwnerOff=${layout.acctOwnerOff}, dataLen=${data.length}`);
    }
    return {
        kind,
 ...
 */
export function parseAllAccounts(data) {
    const indices = parseUsedIndices(data);
    const maxIdx = maxAccountIndex(data.length);
    const validIndices = indices.filter(idx => idx <= maxIdx);
    const droppedCount = indices.length - validIndices.length;
    if (droppedCount > 0) {
        console.warn(`[parseAllAccounts] bitmap claims ${indices.length} used accounts but only ${maxIdx + 1} fit ` +
 ...
 * ```
 */
export function isAccountFlat(account) {
 ...
 * ```
 */
export function filterOpenPositions(accounts) {
 ...
 * ```
 */
export function getSlabHealth(slabData, currentSlot, maxCranknessSlots = 200n) {
    let layout = null;
    try {
        layout = detectSlabLayout(slabData.length, slabData);
        if (!layout)
            return "oracle-unavailable"; // Unrecognized layout — cannot assess health
    }
    catch {
