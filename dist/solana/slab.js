 ...
 */
export const SLAB_TIERS_V_SETDEXPOOL = {};
for (const [label, n] of [["Micro", 64], ["Small", 256], ["Medium", 1024], ["Large", 4096]]) {
...
export function detectSlabLayout(dataLen, data) {
    ...
    if (vsdpn !== undefined)
        return buildLayoutVSetDexPool(vsdpn);
 ...
 */
export function detectLayout(dataLen) {
...
export function parseAllAccounts(data) {
    ...
    if (droppedCount > 0) {
        console.warn(`[parseAllAccounts] bitmap claims ${indices.length} used accounts but only ${maxIdx} fit ` +
