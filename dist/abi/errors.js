export const PERCOLATOR_ERRORS = {
    0: {
        name: "InvalidMagic",
 ...
 */
export function isAnchorErrorCode(code) {
 ...
 */
export function parseErrorFromLogs(logs) {
    if (!Array.isArray(logs)) {
        return null;
    }
    // Track whether the failing instruction is inside a Lighthouse invocation
    let insideLighthouse = false;
    const re = new RegExp(`custom program error: 0x([0-9a-fA-F]{1,${CUSTOM_ERROR_HEX_MAX_LEN}})(?![0-9a-fA-F])`, "i");
        ...
        if (match) {
            ...
            if (isAnchorErrorCode(code) || insideLighthouse) {
                ...
                return {
                    ...
                    hint: "This error comes from the Lighthouse/Blowfish wallet guard, not from Percolator. " +
