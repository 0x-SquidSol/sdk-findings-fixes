 ...
 */
export function computeWarmupUnlockedCapital(totalCapital, currentSlot, warmupStartSlot, warmupPeriodSlots) {
    // No warmup configured or not started → all capital available
    if (warmupPeriodSlots === 0n || warmupStartSlot === 0n)
        return totalCapital;
    if (totalCapital <= 0n)
        return 0n;
 ...
 */
export function computeWarmupLeverageCap(initialMarginBps, totalCapital, currentSlot, warmupStartSlot, warmupPeriodSlots) {
    ...
    if (totalCapital <= 0n)
        return 1;
    const unlocked = computeWarmupUnlockedCapital(totalCapital, currentSlot, warmupStartSlot, warmupPeriodSlots);
    if (unlocked <= 0n)
        return 1; // At least 1x if nothing unlocked yet (slot 0 edge)
    ...
    if (scaledResult > BigInt(Number.MAX_SAFE_INTEGER)) {
        console.warn(`[computeWarmupLeverageCap] Warning: effective leverage ${scaledResult} exceeds MAX_SAFE_INTEGER, ` +
 ...
 */
export function computeWarmupMaxPositionSize(initialMarginBps, totalCapital, currentSlot, warmupStartSlot, warmupPeriodSlots) {
    const maxLev = computeMaxLeverage(initialMarginBps);
    const unlocked = computeWarmupUnlockedCapital(totalCapital, currentSlot, warmupStartSlot, warmupPeriodSlots);
 ...
 * ```
 */
export function computeWarmupProgress(currentSlot, warmupStartedAtSlot, warmupPeriodSlots, pnl, reservedPnl) {
